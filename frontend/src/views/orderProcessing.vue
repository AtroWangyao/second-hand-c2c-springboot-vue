        ...
        <el-table-column
          ...
          <template slot="header" slot-scope="scope">
            ...
            <template #default="scope"  >
                ...
                <el-button @click="OpenDialog(scope.row)" type="text" size="medium" v-if="scope.row.status == 7">调整</el-button>
            </template>
        </el-table-column>
    </el-table>
...
export default {
    ...
    methods: {    
        async handleOffLineMessage(){
            ...
            this.offLineMessageForm.status = 8;//将状态更改为用户面交信息未处理
        ...
        async getOrderList(){
            const{data:result} = await this.$axios.get("order/allOrderBySellerId?sellerId="+getUser().id); 
            this.orderList = result.data;
            for(let i=0;i<this.orderList.length;i++){
                const{data:UserRes} = await this.$axios.get("user/getUserById?userId="+this.orderList[i].buyerId); 
                // console.log(UserRes);
                this.$set(this.orderList[i], 'buyerName', UserRes.data.userName)
            }
        },
